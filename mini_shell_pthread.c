...
Requirements: In Part 2, instead of using fork() to create a child process, we used pthread_create() to create a 
thread and execute the command, after execv finishes, program exits
*/
...
int parsePath(char **dirs) {
   ...
   //Tokenize the path with the dilimter :
   token = strtok(thePath, delimiter);

   //Place the paths in dirs
   while( token != NULL ){
...
void *executeCommand(void *arg) {
    ...
    //reads the PATH variable for the enviornment, then builds an array, dirs[], of the directories in PATH
    parsePath(dirs);

    //Searches the dirctories indentified by the dir argument to see if file name is there
    char *filepath = (char *) malloc(sizeof(char) * CHAR_MAX);
    lookupPath(args, dirs, filepath);

    //execute the command found in the file path along with its parameters, if there are any
...
int main(){
    ...
    while (TRUE) {
            ...
            //Read the PATH variable for the environment, then builds and array, 
            ...
            //If thread creation is successful, pass 'executeCommand' to the thread and execute the command
            //If creation failed, print Error message
            //If execv is successful, program will exit and will not loop again
            threadCheck = pthread_create(&thread, NULL, executeCommand, args);
            if (threadCheck) {
